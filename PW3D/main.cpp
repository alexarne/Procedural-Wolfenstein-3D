...
int main()
{
    ...
    button btn_startgame(0, 0.75, (float)(startY + 0*(height + margin)) / 720, (float) height/720);
    button btn_settings(1, 0.75, (float)(startY + 1*(height + margin)) / 720, (float) height/720);
    button btn_exit(2, 0.75, (float)(startY + 2*(height + margin)) / 720, (float) height/720);
    ...
    while (window.isOpen()) {
        ...
        btn_startgame.draw(window, mouse, !settingsVisible);
        btn_settings.draw(window, mouse, !settingsVisible);
        btn_exit.draw(window, mouse, !settingsVisible);
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                ...
                if (settings.visible()) {
                    ...
                    if (btn_settings.isInside(click))
                        settings.toggle();
                    if (btn_exit.isInside(click))
                        window.close();
                }
            }
